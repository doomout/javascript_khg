...
// 배열 요소 수정하기
// 배열의 요소를 수정할 때는 배열의 인덱스에 새로운 값을 대입하면 된다.
const target4 = ['가', '나', '다', '라', '마'];
target4[3] = '카'; // 배열의 4번째 요소를 '카'로 수정
console.log(target4); // ['가', '나', '다', '카', '마'] - 배열의 4번째 요소가 '카'로 수정됨

// 배열의 요소 삭제하기
// pop() 메서드를 사용하여 배열의 맨 뒤에 있는 요소를 삭제할 수 있다.
const target5 = ['가', '나', '다', '라', '마'];
target5.pop(); // 배열의 맨 뒤에 있는 요소 삭제
console.log(target5); // ['가', '나', '다', '라'] - 배열의 맨 뒤에 있는 요소 삭제됨

// shift() 메서드를 사용하여 배열의 맨 앞에 있는 요소를 삭제할 수 있다.
const target6 = ['가', '나', '다', '라', '마'];
target6.shift(); // 배열의 맨 앞에 있는 요소 삭제
console.log(target6); // ['나', '다', '라', '마'] - 배열의 맨 앞에 있는 요소 삭제됨

// splice() 메서드를 사용하여 배열의 중간에 있는 요소를 삭제할 수 있다.
const target7 = ['가', '나', '다', '라', '마'];
target7.splice(1, 1); // 배열의 2번째 요소 1개 삭제 (나)
console.log(target7); // ['가', '다', '라', '마'] - 배열의 2번째 요소 삭제됨

const target8 = ['가', '나', '다', '라', '마'];
target8.splice(2, 2); // 배열의 2번째 요소부터 2개 삭제 (다, 라)
console.log(target8); // ['가', '나', '마'] - 배열의 2번째 요소부터 2개 삭제됨

// splice() 메서드를 사용하여 배열의 일부를 잘라낼 수 있다.
const target9 = ['가', '나', '다', '라', '마'];
target9.splice(1); // 배열의 2번째 요소부터 끝까지 잘라냄
console.log(target9); // ['가'] - 배열의 2번째 요소부터 끝까지 잘라냄